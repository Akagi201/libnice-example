/*! \file main.cc
 *  \author Akagi201
 *  \brief Negotiate a UDP connection between two clients.
 *
 ...
 *
 *  Build:
 *    gcc -o libnice_simple libnice_simple.c `pkg-config --cflags --libs nice`
 *
 *  Run two clients, one controlling and one controlled:
 *  libnice_simple 0 $(host -4 -t A stun.stunprotocol.org | awk '{ print $4 }')
 *  libnice_simple 1 $(host -4 -t A stun.stunprotocol.org | awk '{ print $4 }')
 */
...
static void cb_new_selected_pair(NiceAgent *agent, guint stream_id,
                                 guint component_id, char *lfoundation,
                                 char *rfoundation, gpointer data);
